...
Проект выполнен как выпускная работа курса Python-разработчика. Основной
фокус моей работы — backend, REST API, модели данных, бизнес-логика,
контейнеризация и CI/CD-деплой.
...
Папка для скриншотов подготовлена в `docs/screenshots/`. После проверки живого
демо или локального запуска туда стоит добавить изображения главной страницы,
карточки рецепта, списка покупок, профиля автора и Redoc-документации.
...
```text
Recipe sharing service with Django REST Framework, PostgreSQL, Docker, Nginx and GitHub Actions CI/CD.
```

Рекомендуемые topics:

```text
python, django, django-rest-framework, postgresql, docker, nginx,
github-actions, rest-api, foodgram, backend
```
